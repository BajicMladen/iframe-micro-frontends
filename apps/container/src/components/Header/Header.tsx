...
const NAV_SEARCH_ITEMS = [
  { label: 'The Must Read', query: 'must read classics' },
  { label: 'News', query: 'new releases 2024' },
  { label: 'Publishers', query: 'award winning books' },
  { label: 'Promotion of the Month', query: 'bestsellers' },
];
      ...
      <nav className='w-full border-b-2 border-purple-400 flex items-center justify-center min-h-16'>
        <div className='w-[80%] flex flex-row justify-between items-center'>
          ...
          <div className='flex flex-row items-center gap-4 text-sm text-gray-400'>
            {['Privacy Policy', 'Warranty', 'Shipping', 'Returns'].map((item) => (
              <div key={item} className='cursor-pointer hover:text-purple-400 transition-colors' onClick={scrollToFooter}>
                {item}
              </div>
            ))}
          </div>
        ...
        <div className='w-[80%] flex flex-row justify-between items-center'>
          <div className='flex flex-row items-center gap-4 text-gray-500'>
            {NAV_SEARCH_ITEMS.map((item) => (
              <div key={item.label} className='cursor-pointer hover:text-purple-400 transition-colors' onClick={() => handleSearch(item.query)}>
                {item.label}
              </div>
            ))}
            <a href='mailto:newsletter@bworld.com' className='cursor-pointer hover:text-purple-400 transition-colors text-gray-500 no-underline'>
              Subscribe to Newsletter
            </a>
          </div>
          <div className='flex flex-row items-center gap-6 text-purple-400'>
            <a href='tel:+12345678901' className='flex flex-row items-center gap-1 text-purple-400 no-underline'>
              <FaPhoneAlt />
              <span className='text-gray-500'>+1 (234) 567-8901</span>
            </a>
            <a href='tel:+12345678901' className='no-underline'>
